function getDepositCount2()
{
	...
	if(dep=="")
	{
		debt=(grand-cash-cheque-cashcd-credit_card1-credit_card2-telegraph_transfer-gift_voucher-debit_card).toFixed(fixnum);
	}
	else if(isNaN(dep))
	{
		alert("The value is not a number. Please try again");
		document.getElementById("deposit").value=dep.substr(0,dep.length-1);
		dep=!isNaN(parseFloat(document.getElementById("deposit").value)) ? parseFloat(document.getElementById("deposit").value) : 0;
