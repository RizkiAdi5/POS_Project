function getDiscountControl()
{
	...
	try
	{
		...
		else if(((d1.value*1)+(d2.value*1)+(d3.value*1))>(document.form1.discountlimit.value*1))
		{
...
function getDiscount2()
{
	...
	if((parseFloat(d1)+parseFloat(d2)+parseFloat(d3))!=0)
	{
		...
		var oriprice = (parseFloat(oldprice)-(parseFloat(totaldiscount)+parseFloat(temp)).toFixed(fixnumdisc)).toFixed(fixnum)*qty;
