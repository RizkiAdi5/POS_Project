function getGift_VoucherCount()
{
	...
	if(gift_voucher=="")
	{
		debt=(grand-cash-cheque-credit_card1-credit_card2-telegraph_transfer-cashcd-dep-debit_card).toFixed(fixnum);
	}
	else if(isNaN(gift_voucher))
	{
		alert("The value is not a number. Please try again");
		document.getElementById("gift_voucher").value=gift_voucher.substr(0,gift_voucher.length-1);
		gift_voucher=!isNaN(parseFloat(document.getElementById("gift_voucher").value)) ? parseFloat(document.getElementById("gift_voucher").value) : 0;
		debt=(grand-cash-cheque-credit_card1-credit_card2-telegraph_transfer-cashcd-gift_voucher-dep-debit_card).toFixed(fixnum);
	}
	else
	{
		debt=(grand-cash-cheque-credit_card1-credit_card2-telegraph_transfer-cashcd-gift_voucher-dep-debit_card).toFixed(fixnum);
