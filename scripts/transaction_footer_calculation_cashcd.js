function getCashCDCount()
{
	var cashcd=document.getElementById("cashcd").value;
	...
	if(cashcd=="")
	{
		debt=(grand-cheque-cash-credit_card1-credit_card2-telegraph_transfer-gift_voucher-dep-debit_card).toFixed(fixnum);
	}
	else if(isNaN(cashcd))
	{
		alert("The value is not a number. Please try again");
		document.getElementById("cashcd").value=cashcd.substr(0,cashcd.length-1);
		cashcd=!isNaN(parseFloat(document.getElementById("cashcd").value)) ? parseFloat(document.getElementById("cashcd").value) : 0;
