...
<cffunction name="listAccount" access="remote" returntype="struct">
    ...
    <cfquery name="getFilteredDataSet" datasource="#dts#">
        SELECT SQL_CALC_FOUND_ROWS kitchenID, name
        FROM kitchen
        <cfif isDefined("form.sSearch") AND form.sSearch NEQ "">
            WHERE (
                kitchenID LIKE <cfqueryparam cfsqltype="cf_sql_varchar" value="%#trim(form.sSearch)#%">
                OR name    LIKE <cfqueryparam cfsqltype="cf_sql_varchar" value="%#trim(form.sSearch)#%">
            )
        </cfif>
        #sOrder#
        #sLimit#
    ...
    <cfquery name="getTotalDataSetLength" datasource="#dts#">
        SELECT COUNT(kitchenID) AS iTotal FROM kitchen
    </cfquery>

    <cfset aaData = ArrayNew(1)>
    <cfloop query="getFilteredDataSet">
        <cfset data = StructNew()>
        <cfset data["kitchenID"] = " " & kitchenID>
