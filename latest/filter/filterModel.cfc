<cfcomponent>
    <cffunction name="listAccount" access="remote" returntype="struct">
        ...
        <cfquery name="listMatchedAccount" datasource="#dts#">
            SELECT model, desp
            FROM vehimodel
            WHERE model LIKE <cfqueryparam cfsqltype="cf_sql_varchar" value="%#term#%" />
        ...
        <cfloop query="listMatchedAccount">
            ...
            <cfset matchedAccount["id"]=evaluate('listMatchedAccount.model')>
            <cfset matchedAccount["model"]=evaluate('listMatchedAccount.model')>
    ...
    <cffunction name="getSelectedAccount" access="remote" returntype="struct">
        ...
        <cfquery name="getSelectedAccount" datasource="#dts#">
            SELECT model, desp
            FROM vehimodel
            WHERE model=<cfqueryparam cfsqltype="cf_sql_varchar" value="#value#" />;
        </cfquery>
        
        <cfset selectedAccount=StructNew()>
        <cfset matchedAccount["id"]=evaluate('getSelectedAccount.model')>
        <cfset selectedAccount["model"]=evaluate('getSelectedAccount.model')>
