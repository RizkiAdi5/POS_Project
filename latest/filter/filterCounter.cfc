<cfcomponent>
    <cffunction name="listAccount" access="remote" returntype="struct">
        ...
        <cfquery name="listMatchedAccount" datasource="#dts#">
            SELECT counterid
            FROM counter
            WHERE counterid LIKE <cfqueryparam cfsqltype="cf_sql_varchar" value="%#term#%" />
            ORDER BY counterid
            LIMIT #start#,#limit#;
        </cfquery>
        ...
        <cfloop query="listMatchedAccount">
            ...
            <cfset matchedAccount["id"]=evaluate('listMatchedAccount.counterid')>
            <cfset matchedAccount["counterid"]=evaluate('listMatchedAccount.counterid')>
    ...
    <cffunction name="getSelectedAccount" access="remote" returntype="struct">
        ...
        <cfquery name="getSelectedAccount" datasource="#dts#">
            SELECT counterid
            FROM counter
            WHERE counterid=<cfqueryparam cfsqltype="cf_sql_varchar" value="#value#" />;
        </cfquery>
        
        <cfset selectedAccount=StructNew()>
        <cfset matchedAccount["id"]=evaluate('listMatchedAccount.counterid')>
        <cfset selectedAccount["counterid"]=evaluate('getSelectedAccount.counterid')>
