  ...
  <cfoutput query="getitem">
	...
	<cfelseif getgeneral.cost eq 'moving'>
		...
		<cfloop query="get1stRC" endrow="1">
			...
			<cfquery name="getpr" datasource="#dts#">
				select sum(amt)as sumamt, sum(qty) as qty, refno, type from ictran where wos_group = "#getitem.wos_group#" and type = 'PR' and wos_date < #get1stRC.wos_date#
...
<cfelseif getgeneral.cost eq "lifo">
	...
	<cfloop query="getitem">
		...
		<cfquery name="getin" datasource="#dts#">
			select sum(qty) as qty from ictran where wos_group = '#getitem.wos_group#' and (type = 'RC' or type = 'CN' or type = 'OAI')
			<cfif form.periodfrom neq "" and form.periodto neq "">
				and fperiod >= '#form.periodfrom#' and fperiod <= '#form.periodto#'
			</cfif>
		</cfquery>
		...
		<cfquery name="getdo" datasource="#dts#">
			select sum(qty) as qty from ictran where wos_group = '#getitem.wos_group#' and type = 'DO' and toinv = ''
			<cfif form.periodfrom neq "" and form.periodto neq "">
				and fperiod >= '#form.periodfrom#' and fperiod <= '#form.periodto#'
			</cfif>
		</cfquery>
		...
		<cfquery name="getout" datasource="#dts#">
			select sum(qty) as qty from ictran where wos_group = '#getitem.wos_group#' and (type = 'INV' or type = 'PR' or type = 'DN' or type = 'CS' or type = 'ISS' or type = 'OAR' or type = 'CT')
			<cfif form.periodfrom neq "" and form.periodto neq "">
				and fperiod >= '#form.periodfrom#' and fperiod <= '#form.periodto#'
			</cfif>
		</cfquery>
...
<cfelseif getgeneral.cost eq "fifo">
    ...
    <cfloop query="getitemdetail">
		...
		<cfquery name="getdo" datasource="#dts#">
			select sum(qty) as qty from ictran where wos_group = '#getitemdetail.wos_group#' and type = 'DO' and (toinv = '' or toinv is null) 
			<cfif form.periodfrom neq "" and form.periodto neq "">
			and fperiod >= '#form.periodfrom#' and fperiod <= '#form.periodto#'
			</cfif>
            and itemno='#getitemdetail.itemno#'
		...
		<cfquery name="getout" datasource="#dts#">
			select sum(qty) as qty from ictran where wos_group = '#getitemdetail.wos_group#' and (type = 'INV' or type = 'PR' or type = 'DN' or type = 'CS' or type = 'ISS' or type = 'OAR')
			<cfif form.periodfrom neq "" and form.periodto neq "">
			and fperiod >= '#form.periodfrom#' and fperiod <= '#form.periodto#'
			</cfif>
            and itemno='#getitemdetail.itemno#'
