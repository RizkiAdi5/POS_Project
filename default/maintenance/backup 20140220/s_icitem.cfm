...
<h4>
    ...
    <a href="itemlocationenquire.cfm" >Item Location Balance Listing</a> ||  
    </cfif>
...
<cfoutput>
	<form action="s_icitem.cfm" method="post">
        ...
        <cfif hcomid eq "kimlee_i" or lcase(HcomID) eq "bakersoven_i">
			...
			<option value="aitemno"><cfif (lcase(HcomID) eq "ideal_i" or lcase(HcomID) eq "idealb_i")>Vendor's Code<cfelse>Product Code</cfif></option>
			<option value="brand">Brand</option>
			<cfelse>
            <option value="All" <cfif getgsetup.ddlitem eq 'All'>selected</cfif>>All</option>	
            <option value="itemno" <cfif getgsetup.ddlitem eq 'Item No'>selected</cfif>><cfif (lcase(HcomID) eq "ideal_i" or lcase(HcomID) eq "idealb_i")>Stock Code<cfelse>Item No</cfif></option>
			<option value="aitemno" <cfif getgsetup.ddlitem eq 'Product Code'>selected</cfif>><cfif (lcase(HcomID) eq "ideal_i" or lcase(HcomID) eq "idealb_i")>Vendor's Code<cfelse>Product Code</cfif></option>
			<option value="brand" <cfif getgsetup.ddlitem eq 'Brand'>selected</cfif>>Brand</option>
