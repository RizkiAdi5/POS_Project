...
<cfif #form.mode# eq "Create">
	<cfquery datasource='#dts#' name="checkitemExist">
 	 	Select * from iccate where cate = '#form.cate#' 
 	 </cfquery>
  	<cfif checkitemExist.recordcount GT 0 >
		<cfoutput>
      		<h3><font color="##FF0000">Error, This #getgeneral.layer# ("#form.cate#") has been created already.</font></h3>
	    </cfoutput> 
    	<cfabort>
	</cfif>
	
	<cfinsert datasource='#dts#' tablename="iccate" formfields="cate,desp">
	
	<cfset status="The #getgeneral.layer#, #form.cate# had been successfully created. ">
		...
		<cfif checkitemExist.recordcount GT 0 >
				...
				<cfif #form.mode# eq "Delete">
						<cfquery datasource='#dts#' name="deleteitem">
								Delete from iccate where cate='#form.cate#'
						</cfquery>
						<cfset status="The #getgeneral.layer#, #form.cate# had been successfully deleted. ">	
								
				</cfif>
				<cfif #form.mode# eq "Edit">
					<cfupdate datasource='#dts#' tablename="iccate" formfields="cate,desp">
...
TAXABLE, TAXPERC1, TAXPERC2, NONSTKITEM, GRAPHIC, PRODCODE, BRK_TO, COLOR, SIZE --->						
						...
						<cfset status="The #getgeneral.layer#, #form.cate# had been successfully edited. ">
				</cfif>
				
		<cfelse>		
			<cfset status="Sorry, the #getgeneral.layer#, #form.cate# was ALREADY removed from the system. Process unsuccessful.">
		</cfif>
</cfif>
<!---Get the messaged to be passed into the view user page. (vUser.cfm) --->

<cfoutput>
	<form name="done" action="s_categorytable.cfm?type=iccate&process=done" method="post">
