...
<cfquery datasource="#dts#" name="getcust">
	select custno,name,name2,currcode from #target_apvend# where (status<>'B' or status is null) 
...
<cfquery datasource="#dts#" name="getinfo">
	select * from icl3p where itemno = <cfqueryparam cfsqltype="cf_sql_char" value="#itemno#"> order by custno
</cfquery>
...
<cfif form.mode eq "delete">
	<cfquery datasource="#dts#" name="deleteicl3p">
		Delete from icl3p where itemno = <cfqueryparam cfsqltype="cf_sql_char" value="#itemno#">
	</cfquery>

	<form name="done" action="suppitemprice.cfm" method="post">
...
<table width="50%" align="center" class="data" cellpadding="2" cellspacing="0">
	<cfoutput>
  	<cfquery datasource="#dts#" name="getdesp">
  			select itemno,desp from icitem where itemno = <cfqueryparam cfsqltype="cf_sql_char" value="#itemno#">
  	</cfquery>
  	...
  	<cfif getinfo.recordcount gt 0>
    	<cfoutput query="getinfo">
      		<cfquery datasource="#dts#" name="getname">
      			select name,currcode from #target_apvend# where custno = '#getinfo.custno#'
      		</cfquery>
			...
			<tr>
        		...
        		<cfif status neq "T">
					...
					<a href="suppitemprice4.cfm?type1=Edit&itemno=#URLEncodedFormat(itemno)#&custno=#URLEncodedFormat(custno)#&mode=#mode#">
					<img height="18px" width="18px" src="../../../images/edit.ICO" alt="Edit" border="0">Edit</a>&nbsp;
					<a href="suppitemprice4.cfm?type1=Delete&itemno=#URLEncodedFormat(itemno)#&custno=#URLEncodedFormat(custno)#&mode=#mode#">
...
<cfform name="form1" method="post" action="suppitemprice.cfm">
...
<cfif status neq "T">
	<cfform name="form" action="suppitemprice4.cfm?type1=Add" method="post" onsubmit="return validate()">
		<cfoutput>
	  ...
	  <hr>
		<table width="85%"  align="center" class="data" cellpadding="2" cellspacing="0">
			<tr>
				...
				<td><input type="submit" name="submit2" value="Add Supplier"></td>
			</tr>
		</table>
	</cfform>
</cfif>
