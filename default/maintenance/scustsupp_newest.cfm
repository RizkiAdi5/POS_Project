...
<cfif getrecordcount.recordcount neq 0>
	...
	<cfform action="scustsupp_newest.cfm?type=#urlencodedformat(type)#" method="post" target="_self">
...
<cfif getgeneral.agentlistuserid eq "Y">and ucase(agent) in (SELECT agent FROM icagent WHERE agentlist like "%#ucase(huserid)#%")
				...
				order by created_on desc,date desc 
				limit #start-1#,20;
			</cfquery>
		<cfelse>
			<cfquery datasource='#dts#' name="getjob">
...
<cfif getgeneral.agentlistuserid eq "Y">and ucase(agent) in (SELECT agent FROM icagent WHERE agentlist like "%#ucase(huserid)#%")
		...
		<cfif start neq 1>
			|| <a target="_self" href="scustsupp_newest.cfm?type=#type#&start=#prevTwenty#">Previous</a> ||
		</cfif>
		
		<cfif page neq noOfPage>
			<a target="_self" href="scustsupp_newest.cfm?type=#type#&start=#nextTwenty#">Next</a> ||
		</cfif>
				...
				<tr onMouseOut="javascript:this.style.backgroundColor='';" onMouseOver="javascript:this.style.backgroundColor='99FF00';" <cfif (type eq "Customer" and getpin2.h1211 eq "T") or (type eq "Supplier" and getpin2.h1111 eq 'T')>ondblclick="javascript:window.parent.location.href('#type#.cfm?type=Edit&custno=#URLEncodedFormat(custno)#');"</cfif>>
					...
					<td nowrap>#getjob.attn#<br/><font style="background-color:FFFFFF">#getjob.e_mail#</font></td>
           			<td nowrap>
						<cfif type eq "Customer" and getpin2.h1211 eq "T">
							<div align="center">
								<a href="#type#.cfm?type=Delete&custno=#URLEncodedFormat(custno)#" target="_parent"><img height="18px" width="18px" src="../../images/delete.ICO" alt="Delete" border="0">Delete</a>&nbsp;
								<a href="#type#.cfm?type=Edit&custno=#URLEncodedFormat(custno)#" target="_parent"><img height="18px" width="18px" src="../../images/edit.ICO" alt="Edit" border="0">Edit</a>
							</div>
						<cfelseif type eq "Supplier" and getpin2.h1111 eq 'T'>
							<div align="center">
								<a href="#type#.cfm?type=Delete&custno=#URLEncodedFormat(custno)#" target="_parent"><img height="18px" width="18px" src="../../images/delete.ICO" alt="Delete" border="0">Delete</a>&nbsp;
