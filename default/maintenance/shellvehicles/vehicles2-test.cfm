...
selectbox.options.add(optn);
	...
	new Ajax.Request(customerurl,
      {
		...
		document.getElementById('customerdetailajax').innerHTML = trim(getdetailback.responseText);
        },
        onFailure: function(){ 
	...
	{
	alert(1);	
	document.getElementById('customerform123').submit();
	}
...
<cfform name='customerform' id="customerform123" action='vehiclesprocess.cfm' method='post'>
  <cfoutput>
  ...
  <table align='center' class='data' width='779' cellspacing="0">
    ...
    <tr>
      ...
      <cfselect name='custcode' id="custcode" onChange="getcustomerdetail();" required="yes" message="Please Select a Customer">
		...
		<input type="text" name="searchsupp" onKeyUp="getSupp('custcode', 'Customer');getcustomerdetail();">
    ...
    <td><textarea name="remark" id="remark" rows="3" cols="60">#remark#</textarea></td>
      ...
      <tr>
        <td align="center" colspan="100%"><input name='submit' type='button' value='#button#' onClick="createprocess();"></td>
      </tr>
