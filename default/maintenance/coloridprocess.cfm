...
<cfif #form.mode# eq "Create">
	<cfquery datasource='#dts#' name="checkitemExist">
	 	 Select * from iccolorid where colorid = '#form.colorid#' 
 	 </cfquery>
  	<cfif checkitemExist.recordcount GT 0 >
		<cfoutput>
      		<h3><font color="##FF0000">Error, This #getgeneral.layer# ("#form.colorid#") has been created already.</font></h3>
	    </cfoutput> 
    	<cfabort>
	</cfif>
	
	<cfinsert datasource='#dts#' tablename="iccolorid" formfields="colorid,desp">
	
	<cfset status="The #getgeneral.layer#, #form.colorid# had been successfully created. ">
		...
		<cfif checkitemExist.recordcount GT 0 >
				...
				<cfif #form.mode# eq "Delete">
						<cfquery datasource='#dts#' name="deleteitem">
								Delete from iccolorid where colorid='#form.colorid#'
						</cfquery>
						<cfset status="The #getgeneral.layer#, #form.colorid# had been successfully deleted. ">	
								
				</cfif>
				<cfif #form.mode# eq "Edit">
					<cfupdate datasource='#dts#' tablename="iccolorid" formfields="colorid,desp">
...
colorid,	FACTOR1, FACTOR2, PRICEU2, UNIT3, FACTORU3_A, FACTORU3_B, PRICEU3, UNIT4, FACTORU4_A, FACTORU4_B,
...
TAXABLE, TAXPERC1, TAXPERC2, NONSTKITEM, GRAPHIC, PRODCODE, BRK_TO, COLOR, SIZE --->						
						...
						<cfset status="The Materail, #form.colorid# had been successfully edited. ">
				</cfif>
				
		<cfelse>		
			<cfset status="Sorry, the #getgeneral.layer#, #form.colorid# was ALREADY removed from the system. Process unsuccessful.">
		</cfif>
</cfif>
<!---Get the messaged to be passed into the view user page. (vUser.cfm) --->

<cfoutput>
	<form name="done" action="s_coloridtable.cfm?type=iccolorid&process=done" method="post">
