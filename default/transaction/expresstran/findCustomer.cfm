...
<cfset nametype = "Cust">
	...
	<cfquery name="getcustsupp" datasource="#dts#">
   		select custno as xcustno,name,add1,add2,add3,add4 from #url.dbtype# <cfif isdefined('url.custno')>WHERE custno like <cfqueryparam cfsqltype="cf_sql_varchar" value="%#url.custno#%"> and (status='' or status is null)<cfelse>where (status='' or status is null)</cfif> limit 15
	</cfquery>
