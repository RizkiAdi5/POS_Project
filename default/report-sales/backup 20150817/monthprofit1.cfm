...
<cfset agenttotal=0>
        ...
        <cfquery name="getbillwithcost" datasource="#dts#">
        	select refno from ictran where type in ('CS','INV') and (void = '' or void is null)
        ...
        <cfquery name="gettotalnorc" datasource="#dts#">
			select sum(invgross) as invgross,sum(roundadj) as roundadj,sum(m_charge1) as m_charge1,sum(discount) as discount,sum(invgross-discount) as net,sum(tax) as tax,sum(grand) as grand,sum(CS_PM_cash) as CS_PM_cash,sum(CS_PM_crcd)+sum(CS_PM_crc2) as CS_PM_crcd,sum(CS_PM_cheq) as CS_PM_cheq,sum(CS_PM_vouc) as CS_PM_vouc,sum(CS_PM_dbcd) as CS_PM_dbcd,sum(CS_PM_cashcd) as CS_PM_cashcd,sum(permitno) as permitno,sum(rem5) as rem5,count(refno) as billcount from artran
			where type in ('CS','INV') and (void = '' or void is null)
			...
			and fperiod = '#form.periodfrom#'
			and wos_date > #getgeneral.lastaccyear#
			and refno not in (<cfqueryparam cfsqltype="cf_sql_varchar" list="yes" separator="," value="#billwithcostlist#">)
		</cfquery>
        
        <cfquery name="gettotalwithrc" datasource="#dts#">
			select sum(invgross) as invgross,sum(roundadj) as roundadj,sum(m_charge1) as m_charge1,sum(discount) as discount,sum(invgross-discount) as net,sum(tax) as tax,sum(grand) as grand,sum(CS_PM_cash) as CS_PM_cash,sum(CS_PM_crcd)+sum(CS_PM_crc2) as CS_PM_crcd,sum(CS_PM_cheq) as CS_PM_cheq,sum(CS_PM_vouc) as CS_PM_vouc,sum(CS_PM_dbcd) as CS_PM_dbcd,sum(CS_PM_cashcd) as CS_PM_cashcd,sum(permitno) as permitno,sum(rem5) as rem5,count(refno) as billcount from artran
			where type in ('CS','INV') and (void = '' or void is null)
			...
			and fperiod = '#form.periodfrom#'
			and wos_date > #getgeneral.lastaccyear#
			and refno in (<cfqueryparam cfsqltype="cf_sql_varchar" list="yes" separator="," value="#billwithcostlist#">)
		</cfquery>
			...
			select sum(it_cos) as ucost from ictran
			where type in ('CS','INV') and (void = '' or void is null)
		...
		<table width="100%" style="font-size:11px; border-width:thin;" cellpadding="0" cellspacing="0" >
			...
			<tr>
				<td colspan="100%"><div align="center"><font size="3" face="Times New Roman, Times, serif"><strong>Monthly Profit Report</strong></font></div></td>
			</tr>
            <tr>
            ...
            <tr>
            	<td><font size="2" face="Times New Roman, Times, serif">Grand Total With Receive :</font></td>
				<td><font size="2" face="Times New Roman, Times, serif"><div align="right">#numberformat(gettotalwithrc.grand,',_.__')#</div></font></td>
                <td>&nbsp;</td>

			</tr>
            <tr>
            	<td><font size="2" face="Times New Roman, Times, serif">Grand Total Without Receive :</font></td>
				<td><font size="2" face="Times New Roman, Times, serif"><div align="right">#numberformat(gettotalnorc.grand,',_.__')#</div></font></td>
                <td>&nbsp;</td>
...
<cfset gettotal.grand=1>
            ...
            <cfset gettotalwithrc.invgross=1>
            </cfif>
            <tr>
