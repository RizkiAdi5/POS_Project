...
<cfswitch expression="#form.result#">
	<cfcase value="EXCELDEFAULT">
		...
		<Styles>
	 	...
	 	<Worksheet ss:Name="Cash_Sales_Report_By_cashier">
	 	<cfoutput>
	   ...
	   <cfif form.periodfrom neq "" and form.periodto neq "">
		...
		<Row ss:AutoFitHeight="0" ss:Height="23.0625">
            ...
            <Cell ss:StyleID="s27"><Data ss:Type="String">Cashier</Data></Cell>
		...
		<cfquery name="getagent" datasource="#dts#">
			select wos_date,refno,cashier,counter,sum(invgross) as invgross,sum(discount) as discount,sum(tax) as tax,sum(grand) as grand,sum(CS_PM_cash) as CS_PM_cash,sum(CS_PM_crcd) as CS_PM_crcd,sum(CS_PM_crc2) as CS_PM_crc2,sum(CS_PM_cheq) as  CS_PM_cheq,sum(CS_PM_vouc) as CS_PM_vouc, sum(deposit) as deposit,sum(CS_PM_dbcd) as CS_PM_dbcd,sum(CS_PM_cashcd) as CS_PM_cashcd,sum(MC1_BIL) as MC1_BIL,cashierid from artran
			where type='CS' and (void = '' or void is null)
			<cfif form.agentfrom neq "" and form.agentto neq "">
			and agenno >='#form.agentfrom#' and agenno <= '#form.agentto#'
			</cfif>
			<cfif form.cashierid neq "">
			and cashierid >='#form.cashierid#'
			</cfif>
			...
			group by wos_date,cashier order by WOS_DATE,cashier
		</cfquery>

		<cfloop query="getagent">
<cfquery name="getcashierlist" datasource="#dts#">
select refno from artran where cashierid='#getagent.cashierid#'
</cfquery>
<cfset cashierlist=valuelist(getcashierlist.refno)>
...
<cfquery name="getictran" datasource="#dts#">
           ...
           <cfif form.locfrom neq "" and form.locto neq "">
            ...
            and wos_date=#getagent.wos_date# and refno in (<cfqueryparam cfsqltype="cf_sql_varchar" list="yes" separator="," value="#cashierlist#">)
                </cfquery>
				...
				<Row ss:AutoFitHeight="0">
					...
					<Cell ss:StyleID="s32"><Data ss:Type="Number">#val(getagent.CS_PM_crcd)#</Data></Cell>
                    ...
                    <Cell ss:StyleID="s32"><Data ss:Type="Number">#val(getagent.deposit)#</Data></Cell>
                     <Cell ss:StyleID="s32"><Data ss:Type="Number">#val(getagent.MC1_BIL)#</Data></Cell>
                      <Cell ss:StyleID="s32"><Data ss:Type="Number">#val(getagent.cashier)#</Data></Cell>
								...
								<cfset totalcrcd = totalcrcd + val(getagent.CS_PM_crcd)>
                                ...
                                <cfset totalmisc = totalmisc + val(getagent.MC1_BIL)>
			   	</Row>
			</cfloop>
	...
	<cfcase value="HTML">
		...
		<cfquery name="getagent" datasource="#dts#">
			select wos_date,refno,cashier,counter,sum(invgross) as invgross,sum(discount) as discount,sum(tax) as tax,sum(grand) as grand,sum(CS_PM_cash) as CS_PM_cash,sum(CS_PM_crcd) as CS_PM_crcd,sum(CS_PM_crc2) as CS_PM_crc2,sum(CS_PM_cheq) as  CS_PM_cheq,sum(CS_PM_vouc) as CS_PM_vouc,sum(CS_PM_dbcd) as CS_PM_dbcd,sum(CS_PM_cashcd) as CS_PM_cashcd, sum(deposit) as deposit, sum(mc1_bil) as MC1_BIL, cashierid from artran
			where type='CS' and (void = '' or void is null)
			<cfif form.agentfrom neq "" and form.agentto neq "">
			and agenno >='#form.agentfrom#' and agenno <= '#form.agentto#'
			</cfif>
		...
		<table width="100%" border="0" cellspacing="0" cellpadding="2">
			<tr>
				<td colspan="100%"><div align="center"><font size="3" face="Times New Roman, Times, serif"><strong>Daily Cash Sales Report By Cashier</strong></font></div></td>
			</tr>
            <tr>
			...
			<tr>
                ...
                <td><div align="right"><font size="2" face="Times New Roman, Times, serif">Cashier</font></div></td>
			</tr>
			<tr>
				<td colspan="100%"><hr></td>
			</tr>
            ...
            and wos_date=#getagent.wos_date# and refno in (<cfqueryparam cfsqltype="cf_sql_varchar" list="yes" separator="," value="#cashierlist#">)
					...
					<tr onMouseOut="javascript:this.style.backgroundColor='';" onMouseOver="javascript:this.style.backgroundColor='99FF00';">
						...
						<td><div align="left"><font size="2" face="Times New Roman, Times, serif">#DateFormat(getagent.WOS_DATE,"dd/mm/yyyy")#</font></div></td>
                                ...
                                <td><div align="right">#numberformat(val(getagent.CS_PM_crcd),',_.__')#</div></td>
                                ...
                                <td><div align="right">#numberformat(val(getagent.MC1_BIL),',_.__')#</div></td>
                                <td><div align="right">#getagent.cashier#</div></td>

								<cfset totalqty = totalqty + val(getictran.qty)>
								<cfset totaldiscount = totaldiscount + val(getagent.discount)>
								<cfset totaltax = totaltax + val(getagent.tax)>
								<cfset totalgrand = totalgrand + val(getagent.grand)>
								<cfset totalcash = totalcash + val(getagent.CS_PM_cash)>
								<cfset totalcrcd = totalcrcd + val(getagent.CS_PM_crcd)>
								<cfset totalcheque = totalcheque + val(getagent.CS_PM_cheq)>
