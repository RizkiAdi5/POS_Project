...
<html>
<head>
<title>Purchase Report Detail By Refno</title>
...
<cfquery datasource="#dts#" name="getbill">
		select refno from artran where (type = 'PO' or type = 'PR') and fperiod <> '99' order by refno
	</cfquery>
...
<h3>
	...
	<a><font size="2">Print Purchase Report Detail by Refno</font></a>
</h3>


<form name="salestype" action="purchasedetailbyrefno1.cfm" method="post" target="_blank">
...
<input type="text" name="searchsuppto" onKeyUp="getSupp('suppto','Supplier');">
    ...
    <tr>
    <th>Include Purchase Return</th>
    <td><input type="checkbox" name="includepr" id="includepr" value="1"></td>
    </tr>
	<tr> 
