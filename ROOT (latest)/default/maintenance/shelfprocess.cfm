...
<cfif #form.mode# eq "Create">
	<cfquery datasource='#dts#' name="checkitemExist">
	 	 Select * from icshelf where shelf = '#form.shelf#' 
 	 </cfquery>
  	<cfif checkitemExist.recordcount GT 0>
		<cfoutput>
      		<h3><font color="##FF0000">Error, This #getgeneral.layer# ("#form.shelf#") has been created already.</font></h3>
	    </cfoutput> 
    	<cfabort>
	</cfif>
	
	<cfinsert datasource='#dts#' tablename="icshelf" formfields="shelf,desp">
	
	<cfset status="The #getgeneral.layer#, #form.shelf# had been successfully created. ">
		...
		<cfif checkitemExist.recordcount GT 0 >
				...
				<cfif #form.mode# eq "Delete">
						<cfquery datasource='#dts#' name="deleteitem">
								Delete from icshelf where shelf='#form.shelf#'
						</cfquery>
						<cfset status="The Size, #form.shelf# had been successfully deleted. ">	
								
				</cfif>
				<cfif #form.mode# eq "Edit">
					<cfupdate datasource='#dts#' tablename="icshelf" formfields="shelf,desp">
					
					<!--- EDI_ID, ITEMNO,	AITEMNO, MITEMNO, SHORTCODE, DESP, DESPA, BRAND, CATEGORY, WOS_GROUP, SHELF, SUPP, PACKING,
WEIGHT,	COSTCODE, UNIT,	UCOST, PRICE, PRICE2, PRICE3, PRICE_MIN, MINIMUM, MAXIMUM, REORDER, UNIT2,shelf,
shelf,	FACTOR1, FACTOR2, PRICEU2, UNIT3, FACTORU3_A, FACTORU3_B, PRICEU3, UNIT4, FACTORU4_A, FACTORU4_B,
...
TAXABLE, TAXPERC1, TAXPERC2, NONSTKITEM, GRAPHIC, PRODCODE, BRK_TO, COLOR, SIZE --->						
						...
						<cfset status="The Materail, #form.shelf# had been successfully edited. ">
				</cfif>
				
		<cfelse>		
			<cfset status="Sorry, the #getgeneral.layer#, #form.shelf# was ALREADY removed from the system. Process unsuccessful.">
		</cfif>
</cfif>
<!---Get the messaged to be passed into the view user page. (vUser.cfm) --->

<cfoutput>
	<form name="done" action="s_shelftable.cfm?type=icshelf&process=done" method="post">
