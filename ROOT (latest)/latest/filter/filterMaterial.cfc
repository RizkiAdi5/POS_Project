<cfcomponent>
    <cffunction name="listAccount" access="remote" returntype="struct">
        ...
        <cfquery name="listMatchedAccount" datasource="#dts#">
            SELECT colorid, desp
            FROM iccolorid
            WHERE colorid LIKE <cfqueryparam cfsqltype="cf_sql_varchar" value="%#term#%" />
        ...
        <cfloop query="listMatchedAccount">
            ...
            <cfset matchedAccount["id"]=evaluate('listMatchedAccount.colorid')>
            <cfset matchedAccount["colorid"]=evaluate('listMatchedAccount.colorid')>
    ...
    <cffunction name="getSelectedAccount" access="remote" returntype="struct">
        ...
        <cfquery name="getSelectedAccount" datasource="#dts#">
            SELECT colorid, desp
            FROM iccolorid
            WHERE colorid=<cfqueryparam cfsqltype="cf_sql_varchar" value="#value#" />;
        </cfquery>
        
        <cfset selectedAccount=StructNew()>
        <cfset matchedAccount["id"]=evaluate('getSelectedAccount.colorid')>
        <cfset selectedAccount["colorid"]=evaluate('getSelectedAccount.colorid')>
