...
<cffunction name="listAccount" access="remote" returntype="struct">
	...
	<cfquery name="getFilteredDataSet" datasource="#dts#">
		SELECT SQL_CALC_FOUND_ROWS custno,name,name2,add1,add2,add3,add4,phone,phonea,attn,currcode
		FROM #targetTable#
		<cfif IsDefined("form.sSearch") AND form.sSearch NEQ "">
            WHERE (
                <cfloop from="0" to="#form.iColumns-1#" index="i" step="1"> 
	...
	<cfloop query="getFilteredDataSet">	
		...
		<cfset data["name2"]=name2>
		<cfset data["add1"]=add1>
		<cfset data["add2"]=add2>
		<cfset data["add3"]=add3>
		<cfset data["add4"]=add4>
		<cfset data["phone"]=phone>
		<cfset data["phonea"]=phonea>
		<cfset data["attn"]=attn>
		<cfset data["currcode"]=currcode>
