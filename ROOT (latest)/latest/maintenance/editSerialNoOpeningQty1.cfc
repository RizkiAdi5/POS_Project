...
<cffunction name="listAccount" access="remote" returntype="struct">
    ...
    <cfset targetItemno=form.itemno>
	...
	<cfquery name="getFilteredDataSet" datasource="#dts#">
		SELECT SQL_CALC_FOUND_ROWS serialno,location,wos_date
		FROM iserial
        WHERE itemno = '#targetItemno#'
        AND type='ADD'
    	AND sign = 1
		<cfif IsDefined("form.sSearch") AND form.sSearch NEQ "">
            AND (
	...
	<cfquery name="getTotalDataSetLength" datasource="#dts#">
		SELECT COUNT(serialno) AS iTotal
		FROM iserial
	</cfquery>
		...
		<cfloop query="getFilteredDataSet">	
			...
			<cfset data["serialno"]=" "&serialno>
			<cfset data["location"]=location>  
            <cfset data["wos_date"]=DateFormat(wos_date,"DD/MM/YYYY")>       
            <cfset data["itemno"]=targetItemno>    
