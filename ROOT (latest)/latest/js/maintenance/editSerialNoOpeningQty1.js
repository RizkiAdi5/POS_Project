				...
				{'aTargets':[3],'sTitle':'ITEM NO','mData':'itemno','bSortable':true,'sWidth':'20%','bVisible':false},
        		{
					...
					'mRender':function(data,type,row){
						return 	'<span class="glyphicon glyphicon-pencil btn btn-link" '+
								'onclick="window.open(\'\/latest\/maintenance\/addSerialNo.cfm?action=update&serialno='+escape(encodeURIComponent(data))+'&location='+escape(encodeURIComponent(row.location))+'&itemno='+escape(encodeURIComponent(row.itemno))+'\',\'_self\');"></span>'+
								'<span class="glyphicon glyphicon-remove btn btn-link" '+
								'onclick="if(confirm(\'Are you sure you wish to delete this serial no?\')){window.open(\'\/latest\/maintenance\/addSerialNoProcess.cfm?action=delete&serialno='+escape(encodeURIComponent(data))+'&location='+escape(encodeURIComponent(row.location))+'&itemno='+escape(encodeURIComponent(row.itemno))+'\',\'_self\');}"></span>';
					}
				}
        	],
			'bAutoWidth':false,
