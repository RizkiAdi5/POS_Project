...
$(document).ready(function(e) {
	...
	var resultTable=$('#resultTable')
		.dataTable({
				...
				{'aTargets':[5],'sTitle':'COMMISSION RATE','mData':'','bSortable':true,'sWidth':'10%'},
				{
					...
					'mRender':function(data,type,row){
						return 	'<span class="glyphicon glyphicon-pencil btn btn-link" '+
								'onclick="window.open(\'\/latest\/maintenance\/commission.cfm?action=update&commid='+escape(encodeURIComponent(data))+'\',\'_self\');"></span>'+
								'<span class="glyphicon glyphicon-remove btn btn-link" '+
								'onclick="if(confirm(\'Are you sure you wish to delete this commission?\')){window.open(\'\/latest\/maintenance\/commissionProcess.cfm?action=delete&commid='+escape(encodeURIComponent(data))+'\',\'_self\');}"></span>';
					}
				}
        	],
			'bAutoWidth':false,
			...
			'sAjaxSource':'/latest/maintenance/commissionProfile.cfc',
			'sServerMethod':'POST',
        	'sScrollX':'100%'
		})
		.fadeIn();
